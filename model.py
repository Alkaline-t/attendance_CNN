...
def predict_with_model(model_data, face_encoding, tolerance=0.60):
    try:
        ...
        if best_similarity >= tolerance:
            ...
            if second_best_similarity > 0 and margin < 0.08:
                logger.warning(f"Insufficient margin: {margin:.3f}. Best: {best_similarity:.3f}, Second: {second_best_similarity:.3f}")
                return None, float(best_similarity)
            
            label = known_labels[best_match_idx]
            return label, float(best_similarity)
        else:
            logger.warning(f"No match. Best similarity: {best_similarity:.3f}")
...
def train_model_background(dataset_dir, progress_callback=None):
    ...
    try:
        ...
        if progress_callback:
            progress_callback(70, f"Preparing model: {len(encodings)} images from {total_students} students", "preparing")
